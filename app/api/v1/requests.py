...
from ...models.inventory import Listing  # <-- ensure this model path is correct
...
def list_my_requests(db: Session = Depends(get_db), current: User = Depends(require_client)):
    ...
    return (
        ...
        .filter(WorkRequest.client_id == prof.id)
        .order_by(WorkRequest.created_at.desc())
        .all()
    )

# Client: create a request (listing_id REQUIRED in your new flow)
...
def create_request(payload: WorkRequestCreate, db: Session = Depends(get_db), current: User = Depends(require_client)):
    ...
    # listing must exist and be active (or NULL status treated as active)
    listing = db.get(Listing, payload.listing_id)
    if not listing or getattr(listing, "status", "active") not in ("active", None):
...
# Provider: browse open requests (MVP)
...
def open_requests(db: Session = Depends(get_db), current: User = Depends(require_provider)):
    return (
        db.query(WorkRequest)
        .filter(WorkRequest.status.in_(["open", "quoted"]))
        .order_by(WorkRequest.created_at.desc())
        .all()
    )
